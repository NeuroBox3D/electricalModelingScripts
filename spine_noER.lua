...
-----------------------------------------------------
-- choice of grid
gridName = util.GetParam("-grid", "grids/spine.ugx")

-- total refinements
numChargeRefs = util.GetParamNumber("-numChargeRefs", 0)
numGlobRefs = util.GetParamNumber("-numGlobRefs", 3)
...
neededSubsets = {"in", "out", "mem", "psd", "mem_in", "mem_out", "psd_in", "psd_out",
	"bnd_noFlux", "bnd_memNoFlux", "bnd_diri", "intfLeft_constr", "intfRight_constr",
...
local extra = "out, bnd_diri, bnd_noFlux"
local intra = "in, extLeft, extRight"
local chSurfIn = "mem_in, psd_in"
local chSurfOut = "mem_out, psd_out"
...
estatPotentialDisc = ConvectionDiffusionFV1(Phi, "mem, psd")
...
for i = 1, #ion_species do
	memSpec:add(0.0, ion_species[i], "mem, psd, bnd_memNoFlux")
end
...
if dimless_surfCh ~= 0 then
	local NeumannBND = UserFluxBoundaryFV1(Phi, chSurfIn..", "..chSurfOut)
...
diriBnds:add(dimless_pot_out, Phi, "bnd_diri, useless")
for i = 1, #ion_species do
	diriBnds:add(dimless_conc_out[i], ion_species[i], "bnd_diri, useless")
end
...
--offsetForGridHierarchy = 5.0
--SaveGridHierarchyTransformed(dom:grid(), dom:subset_handler(), outDir.."grid/refined_grid_hierarchy_lv".. numGlobRefs+numChargeRefs .."_p" .. ProcRank() .. ".ugx", offsetForGridHierarchy)
--SaveParallelGridLayout(dom:grid(), outDir.."grid/parallel_grid_layout_lv".. numGlobRefs+numChargeRefs .."_p"..ProcRank()..".ugx", offsetForGridHierarchy)
...
InterpolateInner(dimless_pot_in, u, "Phi", intra .. ", " .. intfcs .. ", bnd_extLeft, bnd_extRight")

d_phi = 0.5*math.abs(dimless_pot_in)
InterpolateInner(dimless_pot_in-d_phi, u, "Phi", chSurfIn..", mem, bnd_memNoFlux")
InterpolateInner(dimless_pot_out-d_phi, u, "Phi", chSurfOut)
 
for i = 1, #ion_species do
	InterpolateInner(dimless_conc_out[i], u, ion_species[i], extra)
	InterpolateInner(dimless_conc_in[i] , u, ion_species[i], intra .. ", "
...
for i,cat in pairs(cations) do
	InterpolateInner(2*dimless_conc_out[i], u, cat, chSurfOut)
	InterpolateInner(2*dimless_conc_in[i], u, cat, chSurfIn)
end
for i,an in pairs(anions) do
	InterpolateInner(0.5*dimless_conc_out[i], u, an, chSurfOut)
	InterpolateInner(0.5*dimless_conc_in[i], u, an, chSurfIn)
end
...
function channelOpen(x,y,z,t)
	-- strong NMDAR
	--return math.exp(-t/tau_dimless_nmdar)
	
	-- tetanic stimulation
	local t_p = t % period_dimless
	--return math.exp(-t_p / tau_dimless)
...
kOutAtSyn = dimless_conc_out[1]
naInAtSyn = dimless_conc_in[3]
naOutAtSyn = dimless_conc_out[3]
...
function psd_flux_k_in(x,y,z,t,si)
	...
	return - channelOpen(0,0,0,t) * PSD_channel_dens[1] * dimless_diff_const[1] / 1e-8 * len_ref *
		F/(R*T) * vmAtSyn * (kOutAtSyn - kInAtSyn * exp) / (1.0 - exp)
end
function psd_flux_k_out(x,y,z,t,si)
	return - psd_flux_k_in(x,y,z,t,si)
end

function psd_flux_na_in(x,y,z,t,si)
	local exp = math.exp(F/(R*T) * vmAtSyn)
	return - channelOpen(0,0,0,t) * PSD_channel_dens[3] * dimless_diff_const[3] / 1e-8 * len_ref *
		F/(R*T) * vmAtSyn * (naOutAtSyn - naInAtSyn * exp) / (1.0 - exp)
end
function psd_flux_na_out(x,y,z,t,si)
	return - psd_flux_na_in(x,y,z,t,si)
end


PSD_NeumannKInner = UserFluxBoundaryFV1(ion_species[1], "psd_in")
PSD_NeumannKInner:set_flux_function("psd_flux_k_in")
PSD_NeumannKOuter = UserFluxBoundaryFV1(ion_species[1], "psd_out")
PSD_NeumannKOuter:set_flux_function("psd_flux_k_out")
domainDisc:add(PSD_NeumannKInner)
domainDisc:add(PSD_NeumannKOuter)

PSD_NeumannNaInner = UserFluxBoundaryFV1(ion_species[3], "psd_in")
PSD_NeumannNaInner:set_flux_function("psd_flux_na_in")
PSD_NeumannNaOuter = UserFluxBoundaryFV1(ion_species[3], "psd_out")
PSD_NeumannNaOuter:set_flux_function("psd_flux_na_out")
domainDisc:add(PSD_NeumannNaInner)
domainDisc:add(PSD_NeumannNaOuter)
--]]
...
function preTimestepActions(step, time, dt)
	...
	vmAtSyn = pot_ref * (Integral(curSol, Phi, "psd_in") / psdInArea
						 - Integral(curSol, Phi, "psd_out") / psdOutArea)
	
	---[[
	kInAtSyn = Integral(curSol, "K", "psd_in") / psdInArea
	kOutAtSyn = Integral(curSol, "K", "psd_out") / psdOutArea
	naInAtSyn = Integral(curSol, "Na", "psd_in") / psdInArea
	naOutAtSyn = Integral(curSol, "Na", "psd_out") / psdOutArea
	--]]
