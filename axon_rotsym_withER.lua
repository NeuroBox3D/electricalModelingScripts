...
-- Inside the axon, a concentric cylinder of about 1/10 the neuron's volume   --
-- is inserted as a kind of obstacle or additional condenser).                --
...
------------------------------------
-- grid
grid = util.GetParam("-grid", "grids/axon5_2d_er.ugx")
...
numAnisoRefs = util.GetParamNumber("-numAnisoRefs", 0, "number of anisotropic refinements")
...
-- choose whether the ER is to be treated as pure obstacle or as capacitor
erAsPureObstacle = util.HasParamOption("-erAsPureObstacle")
...
-------------------------------
neededSubsets = {"in", "out", "er", "myelin", "ermem", "bnd", "ranvier1"}
for i = 2,nRanvier do
...
if loadBalancer ~= nil then
	if balancer.partitioner == "parmetis" then
		...
		mu:add_protectable_subsets("ermem")
...
approxSpace:add_fct("phiER", "Lagrange", 1, volER)
...
function scaled_conductivity_er(z,r,t)
	local s = 2*math.pi*r * sigma_er
	return  s, 0,
			0, s
end
...
-- ER potential
elemDiscPhiER = ConvectionDiffusion("phiER", "er", "fv1")
elemDiscPhiER:set_diffusion("scaled_conductivity_er")
elemDiscPhiER:set_mass_scale(0.0)
...
elemDiscCapERMOut = ConvectionDiffusion("phiIn", "ermem", "fv1")
elemDiscCapERMOut:set_mass_scale(ranvierCapData)
linkerCapERMOut = ScaleAddLinkerNumber()
linkerCapERMOut:add(ranvierCapDataNeg, elemDiscPhiER:value())
elemDiscCapERMOut:set_mass(linkerCapERMOut)

elemDiscCapERMIn = ConvectionDiffusion("phiER", "ermem", "fv1")
elemDiscCapERMIn:set_mass_scale(ranvierCapData)
linkerCapERMIn = ScaleAddLinkerNumber()
linkerCapERMIn:add(ranvierCapDataNeg, elemDiscPhiIn:value())
elemDiscCapERMIn:set_mass(linkerCapERMIn)
...
if erAsPureObstacle then
	dirichletBndPhiOut:add(0.0, "phiER", "er")
	dirichletBndPhiOut:add(0.0, "phiER", "ermem")
end
...
if not erAsPureObstacle then
	domainDiscImpl:add(elemDiscPhiER)
	domainDiscImpl:add(elemDiscCapERMOut)
	domainDiscImpl:add(elemDiscCapERMIn)
end
...
InterpolateInner(phi_eq_in, u, "phiER", volER, time)
